...
class GraphBuilder:
    ...
    def build_graph(
        self, resources: list[AzureResource], network_topology: NetworkTopology
    ) -> nx.DiGraph:
    ...
    def _filter_compute_only(
        ...
        compute_resource_groups = {
            resource.resource_group for resource in compute_resources
        }
        ...
        for resource in resources:
            if (
                ...
                if (
                    resource.resource_group in compute_resource_groups
                    or resource.resource_type.lower() == "internet/gateway"
                ):
        ...
        for compute_resource in compute_resources:
            if (
                compute_resource.resource_type.lower()
                == "microsoft.compute/virtualmachines"
            ):
                ...
                for resource in resources:
                    if (
                        ...
                        if (
                            vm_name in resource_name
                            or resource_name in vm_name
                            or any(
                                vm_name.replace(sep, "")
                                == resource_name.replace(sep, "")
                                for sep in ["-", "_", " "]
                            )
                            or any(
                                vm_name.startswith(prefix)
                                and resource_name.startswith(prefix)
                                for prefix in [vm_name[:4], vm_name[:6]]
                                if len(prefix) >= 3
                            )
                        ):
                            related_resources.append(resource)
    ...
    def _create_group_node(
        self, group_key: str, resources: list[AzureResource]
    ) -> GraphNode:
        ...
        label = self._build_node_label(
            group_key, resource_names, resources[0].category, group_key
        )
    ...
    def _create_resource_node(self, resource: AzureResource) -> GraphNode:
        ...
        resource_type_suffix = (
            resource.resource_type.split("/")[-1].lower()
            if "/" in resource.resource_type
            else resource.resource_type.lower()
        )
        node_id = (
            f"{resource.category.lower()}_{resource.name.lower()}_{resource_type_suffix}".replace(
                " ", "_"
            )
            .replace("-", "_")
            .replace(".", "_")
        ...
        label = self._build_node_label(
            resource.name, [resource.name], resource.category, resource.resource_type
        )
    ...
    def _create_network_edges(
        self, network_topology: NetworkTopology, resources: list[AzureResource]
    ):
        ...
        for r in resources:
            ...
            resource_type_suffix = (
                r.resource_type.split("/")[-1].lower()
                if "/" in r.resource_type
                else r.resource_type.lower()
            )
        ...
        for association in network_topology.associations:
            ...
            if source_id in resource_by_id:
                ...
                for key, resource in resource_by_id.items():
                    if (
                        key.startswith(f"{source_name}_")
                        and resource.name == source_name
                    ):
            ...
            if target_id in resource_by_id:
                ...
                for key, resource in resource_by_id.items():
                    if (
                        key.startswith(f"{target_name}_")
                        and resource.name == target_name
                    ):
            ...
            if source_resource and target_resource:
                ...
                source_type_suffix = (
                    source_resource.resource_type.split("/")[-1].lower()
                    if "/" in source_resource.resource_type
                    else source_resource.resource_type.lower()
                )
                target_type_suffix = (
                    target_resource.resource_type.split("/")[-1].lower()
                    if "/" in target_resource.resource_type
                    else target_resource.resource_type.lower()
                )
                edge = GraphEdge(
                    source=f"{source_resource.category.lower()}_{source_resource.name.lower()}_{source_type_suffix}".replace(
                        " ", "_"
                    )
                    .replace("-", "_")
                    .replace(".", "_"),
                    target=f"{target_resource.category.lower()}_{target_resource.name.lower()}_{target_type_suffix}".replace(
                        " ", "_"
                    )
                    .replace("-", "_")
                    .replace(".", "_"),
                    ...
                    attributes={"style": "solid", "color": "blue"},
                )
        ...
        for resource in resources:
            for dependency in resource.dependencies:
                ...
                if dep_name in resource_by_name:
                    ...
                    target_type_suffix = (
                        dep_resource.resource_type.split("/")[-1].lower()
                        if "/" in dep_resource.resource_type
                        else dep_resource.resource_type.lower()
                    )
                    edge = GraphEdge(
                        source=f"{resource.category.lower()}_{resource.name.lower()}_{source_type_suffix}".replace(
                            " ", "_"
                        )
                        .replace("-", "_")
                        .replace(".", "_"),
                        target=f"{dep_resource.category.lower()}_{dep_name.lower()}_{target_type_suffix}".replace(
                            " ", "_"
                        )
                        .replace("-", "_")
                        .replace(".", "_"),
    ...
    def _create_dns_zone_connections(
        self, resources: list[AzureResource], resource_by_name: dict[str, AzureResource]
    ):
        ...
        for dns_zone in dns_zones:
            ...
            if zone_name_parts:
                ...
                for resource, is_pattern_match in matching_resources:
                    ...
                    # Use same node ID generation logic as _create_resource_node
                    source_type_suffix = (
                        dns_zone.resource_type.split("/")[-1].lower()
                        if "/" in dns_zone.resource_type
                        else dns_zone.resource_type.lower()
                    )
                    target_type_suffix = (
                        resource.resource_type.split("/")[-1].lower()
                        if "/" in resource.resource_type
                    ...
                    edge = GraphEdge(
                        source=f"{dns_zone.category.lower()}_{dns_zone.name.lower()}_{source_type_suffix}".replace(
                            " ", "_"
                        )
                        .replace("-", "_")
                        .replace(".", "_"),
                        target=f"{resource.category.lower()}_{resource.name.lower()}_{target_type_suffix}".replace(
                            " ", "_"
                        )
                        .replace("-", "_")
                        .replace(".", "_"),
    ...
    def _filter_redundant_edges(self, edges: list[GraphEdge]) -> list[GraphEdge]:
        ...
        edge_pairs: dict[tuple[str, str], bool] = {}
        filtered_edges = []
        ...
        for pair_key, pair_edges in edge_pairs.items():
            if len(pair_edges) == 1:
                ...
                edge1_source_node = next(
                    (node for node in self.nodes if node.id == edge1.source), None
                )
                edge1_target_node = next(
                    (node for node in self.nodes if node.id == edge1.target), None
                )
                edge2_source_node = next(
                    (node for node in self.nodes if node.id == edge2.source), None
                )
                edge2_target_node = next(
                    (node for node in self.nodes if node.id == edge2.target), None
                )
                ...
                if self._is_resource_type(
                    edge1_source_node, "microsoft.network/networkinterfaces"
                ) and self._is_resource_type(
                    edge1_target_node, "microsoft.compute/virtualmachines"
                ):
                    kept_edge = edge1
                elif self._is_resource_type(
                    edge2_source_node, "microsoft.network/networkinterfaces"
                ) and self._is_resource_type(
                    edge2_target_node, "microsoft.compute/virtualmachines"
                ):
                    kept_edge = edge2
                ...
                # NIC → Public IP (remove Public IP → NIC)
                    ...
                    edge1_target_node, "microsoft.network/publicipaddresses"
                ):
                    kept_edge = edge1
                elif self._is_resource_type(
                    edge2_source_node, "microsoft.network/networkinterfaces"
                ) and self._is_resource_type(
                    edge2_target_node, "microsoft.network/publicipaddresses"
                ):
                    kept_edge = edge2
    ...
    def _populate_networkx_graph(self) -> None:
        ...
        for node in self.nodes:
            ...
            if "properties" in node.attributes and isinstance(
                node.attributes["properties"], dict
            ):
        ...
        for edge in filtered_edges:
            self.graph.add_edge(
                edge.source,
                edge.target,
                **{"label": edge.label, "edge_type": edge.edge_type, **edge.attributes},
            )
