...
class GraphBuilder:
    ...
    def _build_node_label(self, name: str, resource_names: List[str], category: str, resource_type: str = None) -> str:
        ...
        if resource_type and resource_type.lower() == "microsoft.compute/galleries/images":
            if self.config.label_verbosity == LabelVerbosity.MINIMAL:
        ...
        if resource_type and resource_type.lower() == "microsoft.compute/galleries/images/versions":
            if self.config.label_verbosity == LabelVerbosity.MINIMAL:
        ...
        if resource_type and resource_type.lower() == "microsoft.managedidentity/userassignedidentities":
            if self.config.label_verbosity == LabelVerbosity.MINIMAL:
                return name
            if self.config.label_verbosity == LabelVerbosity.STANDARD:
                return f"{name}\\n(Managed Identity)"
        ...
        if resource_type and resource_type.lower() == "microsoft.network/privatednszones":
            if self.config.label_verbosity == LabelVerbosity.MINIMAL:
                return name
            if self.config.label_verbosity == LabelVerbosity.STANDARD:
                return f"{name}\\n(Private DNS Zone)"
        ...
        if resource_type and resource_type.lower() == "microsoft.network/privatednszones/virtualnetworklinks":
            if self.config.label_verbosity == LabelVerbosity.MINIMAL:
            ...
            for dependency in resource.dependencies:
                ...
                if dep_name in resource_by_name:
                    ...
                    # For VNet-subnet dependencies, create a connection to show containment
                          ...
                          dep_resource.resource_type == "Microsoft.Network/virtualNetworks/subnets") or (resource.resource_type == "Microsoft.Network/virtualNetworks" and
                    ...
                    # For DNS zone dependencies, create DNS resolution connections
                          ...
                          dep_resource.resource_type == "Microsoft.Network/loadBalancers") or (resource.resource_type == "Microsoft.Network/dnszones" and
                          dep_resource.resource_type == "Microsoft.Network/publicIPAddresses"):
                        edge_attrs = {
                            "style": "solid",
                            "color": "navy",
