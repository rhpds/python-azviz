...
class AzureClient:
    ...
    def __init__(
        ...
        if not subscription_identifier:
            ...
            self.subscription_id, self.subscription_name = (
                self._resolve_subscription_identifier(subscription_identifier)
            )
        ...
        logger.info(
            f"Initialized Azure client for subscription: {self.subscription_name} ({self.subscription_id})",
        )
    ...
    def _resolve_subscription_identifier(
        ...
        try:
            ...
            if len(partial_matches) > 1:
                ...
                raise ValueError(
                    f"Multiple subscriptions match '{subscription_identifier}': {matches_str}. Please be more specific.",
                )
            available_subs = ", ".join(
                [f"'{sub.display_name}'" for sub in subscriptions[:5]],
            )  # Show first 5
            raise ValueError(
                f"Subscription '{subscription_identifier}' not found. Available subscriptions: {available_subs}",
            )

        except AzureError as e:
            raise ValueError(
                f"Failed to resolve subscription '{subscription_identifier}': {e}",
            ) from e
    ...
    def get_resources_in_group(
        ...
        try:
            ...
            logger.info(
                f"Found {len(resources)} resources in group '{resource_group_name}'",
            )
            return resources
        except AzureError as e:
            logger.error(
                f"Failed to get resources for group '{resource_group_name}': {e}",
            )
            raise
        ...
        try:
            ...
            for vm in vms:
                try:
                    ...
                    # OS disk
                    if (
                        vm_details.storage_profile
                        and vm_details.storage_profile.os_disk
                    ):
                        os_disk = vm_details.storage_profile.os_disk
                        if os_disk.managed_disk and os_disk.managed_disk.id:
                            disk_name = self._extract_resource_name_from_id(
                                os_disk.managed_disk.id,
                            )
                    ...
                    if (
                        vm_details.storage_profile
                        and vm_details.storage_profile.data_disks
                    ):
                    ...
                    for disk in disks:
                        if disk.name in attached_disk_names:
                            vm.add_dependency(
                                ...
                                "Azure API - disk attachment",
                            )
                            logger.debug(
                                f"Added disk dependency: {vm.name} -> {disk.name}",
                            )

                except Exception as e:
                    logger.warning(
                        f"Could not get disk details for VM '{vm.name}': {e}",
                    )
                    continue
    ...
    def _discover_vm_ssh_key_relationships(
        ...
        ssh_keys = [
            r for r in resources if r.resource_type == "Microsoft.Compute/sshPublicKeys"
        ]
                ...
                try:
                    ...
                    for vm in vms:
                        try:
                            ...
                            if (
                                vm_details.os_profile
                                and vm_details.os_profile.linux_configuration
                                ...
                                for public_key_config in vm_details.os_profile.linux_configuration.ssh.public_keys:
                                    if (
                                        ...
                                        vm.add_dependency(
                                            ssh_key.name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - SSH key configuration",
                                        )
                                        logger.debug(
                                            f"Added SSH key dependency: {vm.name} -> {ssh_key.name}",
                                        )
                                        break

                        except Exception as e:
                            logger.warning(
                                f"Could not get VM details for SSH key analysis '{vm.name}': {e}",
                            )
                            continue

                except Exception as e:
                    logger.warning(
                        f"Could not get SSH key details for '{ssh_key.name}': {e}",
                    )
                    continue
    ...
    def _discover_gallery_relationships(self, resources: List[AzureResource]) -> None:
        ...
        galleries = [
            r for r in resources if r.resource_type == "Microsoft.Compute/galleries"
        ]
        gallery_images = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Compute/galleries/images"
        ]
        gallery_versions = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Compute/galleries/images/versions"
        ]
                ...
                if "/" in gallery_image.name:
                    ...
                    for gallery in galleries:
                        if gallery.name == gallery_name:
                            gallery_image.add_dependency(
                                gallery.name,
                                DependencyType.EXPLICIT,
                                "Azure API - gallery hierarchy",
                            )
                            logger.debug(
                                f"Added gallery dependency: {gallery_image.name} -> {gallery.name}",
                            )
                            break
                ...
                if len(name_parts) >= 3:
                    ...
                    for gallery_image in gallery_images:
                        if gallery_image.name == gallery_image_name:
                            gallery_version.add_dependency(
                                gallery_image.name,
                                DependencyType.EXPLICIT,
                                "Azure API - gallery hierarchy",
                            )
                            logger.debug(
                                f"Added gallery image dependency: {gallery_version.name} -> {gallery_image.name}",
                            )
                            break
    ...
    def _discover_managed_identity_relationships(
        ...
        managed_identities = [
            r
            for r in resources
            if r.resource_type == "Microsoft.ManagedIdentity/userAssignedIdentities"
        ]
        potential_users = [
            r
            for r in resources
        ...
        try:
            ...
            for resource in potential_users:
                try:
                    ...
                    if (
                        resource_details
                        and hasattr(resource_details, "identity")
                        and resource_details.identity
                    ):
                        ...
                        if (
                            ...
                            for (
                                identity_id,
                                _,
                            ) in identity_obj.user_assigned_identities.items():
                                # Extract identity name from resource ID
                                identity_name = self._extract_resource_name_from_id(
                                    identity_id,
                                )
                                ...
                                for managed_identity in managed_identities:
                                    if managed_identity.name == identity_name:
                                        resource.add_dependency(
                                            managed_identity.name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - managed identity assignment",
                                        )
                                        logger.debug(
                                            f"Added managed identity dependency: {resource.name} -> {managed_identity.name}",
                                        )
                                        break

                        # Check for system-assigned identity (would be implicit, no explicit dependency)
                        if (
                            hasattr(identity_obj, "type")
                            and identity_obj.type
                            and "SystemAssigned" in str(identity_obj.type)
                        ):
                            logger.debug(
                                f"Resource {resource.name} uses system-assigned managed identity",
                            )

                except Exception as e:
                    logger.warning(
                        f"Could not get managed identity details for '{resource.name}': {e}",
                    )
                    continue
    ...
    def _discover_private_dns_relationships(
        ...
        private_dns_zones = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/privateDnsZones"
        ]
        vnet_links = [
            r
            for r in resources
        ...
        vnets = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/virtualNetworks"
        ]
        ...
        try:
                ...
                if "/" in vnet_link.name:
                    ...
                    for dns_zone in private_dns_zones:
                        if dns_zone.name == dns_zone_name:
                            vnet_link.add_dependency(
                                dns_zone.name,
                                DependencyType.EXPLICIT,
                                "Azure API - private DNS zone link",
                            )
                            logger.debug(
                                f"Added Private DNS Zone dependency: {vnet_link.name} -> {dns_zone.name}",
                            )
                            break
            ...
            for vnet_link in vnet_links:
                try:
                    ...
                    dns_zone_name = (
                        vnet_link.name.split("/")[0]
                        if "/" in vnet_link.name
                        else vnet_link.name
                    )
                    link_name = (
                        vnet_link.name.split("/")[-1]
                        if "/" in vnet_link.name
                        else vnet_link.name
                    )
                    ...
                    try:
                        ...
                        if (
                            hasattr(vnet_link_details, "virtual_network")
                            and vnet_link_details.virtual_network
                        ):
                            ...
                            for vnet in vnets:
                                if vnet.name == vnet_name:
                                    vnet_link.add_dependency(
                                        vnet.name,
                                        DependencyType.EXPLICIT,
                                        "Azure API - VNet link connection",
                                    )
                                    logger.debug(
                                        f"Added VNet dependency: {vnet_link.name} -> {vnet.name}",
                                    )
                                    break
                        ...
                        if result.returncode == 0:
                            ...
                            if (
                                "ResourceNotFound" in error_output
                                or "ResourceGroupNotFound" in error_output
                                or "exit status 3" in str(error_output)
                                or "could not be found" in error_output
                            ):
                                logger.debug(
                                    f"VNet link '{vnet_link.name}' references non-existent resource group or has been deleted - skipping",
                                )
                            else:
                                logger.warning(
                                    f"Could not get VNet link details for '{vnet_link.name}': {result.stderr}",
                                )

                except Exception as e:
                    logger.debug(
                        f"Could not get VNet link details for '{vnet_link.name}': {e}",
                    )
                    continue
            ...
            for private_dns_zone in private_dns_zones:
                ...
                zone_vnet_links = [
                    vl
                    for vl in vnet_links
                    if vl.name.startswith(f"{private_dns_zone.name}/")
                ]
                    ...
                    for dep_name in zone_vnet_link.dependencies:
                        ...
                        for vnet in vnets:
                            if vnet.name == dep_name:
                                private_dns_zone.add_dependency(
                                    vnet.name,
                                    DependencyType.EXPLICIT,
                                    "Azure API - DNS resolution for VNet",
                                )
                                logger.debug(
                                    f"Added VNet dependency for DNS resolution: {private_dns_zone.name} -> {vnet.name}",
                                )
                                break
    ...
    def _discover_route_table_relationships(
        ...
        route_tables = [
            r for r in resources if r.resource_type == "Microsoft.Network/routeTables"
        ]
        subnets = [
            r
            for r in resources
        ...
        try:
            ...
            for route_table in route_tables:
                try:
                    ...
                    if (
                        hasattr(route_table_details, "subnets")
                        and route_table_details.subnets
                    ):
                        for subnet_ref in route_table_details.subnets:
                            if subnet_ref.id:
                                ...
                                if len(id_parts) >= 11 and "subnets" in id_parts:
                                    ...
                                    if vnet_index + 1 < len(
                                        id_parts,
                                    ) and subnet_index + 1 < len(id_parts):
                                        ...
                                        for subnet in subnets:
                                            if subnet.name == full_subnet_name:
                                                subnet.add_dependency(
                                                    route_table.name,
                                                    DependencyType.EXPLICIT,
                                                    "Azure API - route table association",
                                                )
                                                logger.debug(
                                                    f"Added route table dependency: {subnet.name} -> {route_table.name}",
                                                )
                                                break

                except Exception as e:
                    logger.warning(
                        f"Could not get route table details for '{route_table.name}': {e}",
                    )
                    continue
    ...
    def _discover_dns_zone_relationships(self, resources: List[AzureResource]) -> None:
        ...
        try:
            ...
            load_balancers = [
                r
                for r in resources
                if r.resource_type == "Microsoft.Network/loadBalancers"
            ]
            public_ips = [
                r
                for r in resources
                        ...
                        if record.get("metadata"):
                            for key, value in record["metadata"].items():
                                if "kubernetes.io_cluster." in key and value == "owned":
                                    ...
                                    cluster_name = key.replace(
                                        "kubernetes.io_cluster.",
                                        "",
                                    )
                                    cluster_names.add(cluster_name)
                                    logger.debug(
                                        f"DNS zone '{dns_zone.name}' serves cluster '{cluster_name}'",
                                    )
                    ...
                    for cluster_name in cluster_names:
                        ...
                        for lb in all_load_balancers:
                            if cluster_name in lb.name.lower():
                                dns_zone.add_dependency(
                                    lb.name,
                                    DependencyType.DERIVED,
                                    "DNS cluster name pattern match",
                                )
                                logger.debug(
                                    f"Added DNS-LoadBalancer dependency: {dns_zone.name} -> {lb.name} (derived)",
                                )
                        ...
                        for pip in all_public_ips:
                            if cluster_name in pip.name.lower():
                                ...
                                logger.debug(
                                    f"Added DNS-PublicIP dependency: {dns_zone.name} -> {pip.name} (derived)",
                                )
                        ...
                        for master_vm in all_master_vms:
                            if cluster_name in master_vm.name.lower():
                                ...
                                logger.debug(
                                    f"Added DNS-Master dependency: {dns_zone.name} -> {master_vm.name} (derived)",
                                )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze DNS zone records for '{dns_zone.name}': {e}",
                    )
                    continue
    ...
    def _discover_nic_private_endpoint_relationships(
        ...
        nics = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/networkInterfaces"
        ]
        private_endpoints = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/privateEndpoints"
        ]
        private_link_services = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/privateLinkServices"
        ]
        ...
        try:
            ...
            for nic in nics:
                try:
                    ...
                    if (
                        hasattr(nic_details, "private_endpoint")
                    ...
                    # Check if this NIC belongs to a private link service
                        ...
                        hasattr(nic_details, "private_link_service")
            ...
            logger.warning(
                f"Failed to discover NIC-private endpoint/private link service relationships: {e}",
            )
    ...
    def _discover_private_link_service_relationships(
        ...
        load_balancers = [
            r for r in resources if r.resource_type == "Microsoft.Network/loadBalancers"
        ]
        ...
        try:
            ...
            for pls in private_link_services:
                try:
                    ...
                    if (
                        hasattr(pls_details, "load_balancer_frontend_ip_configurations")
                        and pls_details.load_balancer_frontend_ip_configurations
                    ):
                        for (
                            frontend_config
                        ) in pls_details.load_balancer_frontend_ip_configurations:
                                        ...
                                        for lb in load_balancers:
                                            if lb.name == lb_name:
                                                pls.add_dependency(
                                                    lb.name,
                                                    DependencyType.EXPLICIT,
                                                    "Azure API - private link service load balancer",
                                                )
                                                logger.debug(
                                                    f"Added load balancer dependency: {pls.name} -> {lb.name}",
                                                )
                                                break

                except Exception as e:
                    logger.warning(
                        f"Could not get private link service details for '{pls.name}': {e}",
                    )
                    continue
                    ...
                    if hasattr(vnet_details, "subnets") and vnet_details.subnets:
                        for subnet in vnet_details.subnets:
                            ...
                            if not subnet_exists:
                                ...
                                logger.debug(
                                    f"Created virtual subnet resource: {subnet_full_name} (prefix: {getattr(subnet, 'address_prefix', 'unknown')})",
                                )
        ...
        try:
            ...
            for pe in private_endpoints:
                try:
                    ...
                    if (
                        hasattr(pe_details, "subnet")
                        and pe_details.subnet
                        and pe_details.subnet.id
                    ):
                        ...
                        if (
                            len(id_parts) >= 11
                            and "virtualNetworks" in id_parts
                            and "subnets" in id_parts
                        ):
                            vnet_index = id_parts.index("virtualNetworks")
                            subnet_index = id_parts.index("subnets")
                            ...
                            if vnet_index + 1 < len(
                                ...
                                pe.add_dependency(
                                    subnet_full_name,
                                    DependencyType.EXPLICIT,
                                    "Azure API - private endpoint subnet placement",
                                )
                                logger.debug(
                                    f"Added subnet dependency: {pe.name} -> {subnet_full_name}",
                                )
                    ...
                    if (
                        hasattr(pe_details, "private_link_service_connections")
                        and pe_details.private_link_service_connections
                    ):
                        pls_connections.extend(
                            pe_details.private_link_service_connections,
                        )

                    if (
                        hasattr(pe_details, "manual_private_link_service_connections")
                        and pe_details.manual_private_link_service_connections
                    ):
                        pls_connections.extend(
                            pe_details.manual_private_link_service_connections,
                        )
                    ...
                    for conn in pls_connections:
                        if (
                            hasattr(conn, "private_link_service_id")
                            and conn.private_link_service_id
                        ):
                            ...
                            if len(id_parts) >= 5:
                                pls_rg = id_parts[4]
                                pls_subscription = (
                                    id_parts[2] if len(id_parts) >= 3 else None
                                )
                                ...
                                logger.debug(
                                    f"Added external PLS connection: {pe.name} -> {pls_name} (RG: {pls_rg})",
                                )
                    ...
                    logger.warning(
                        f"Could not get private endpoint details for '{pe.name}': {e}",
                    )
                    continue
        ...
        try:
            ...
            for nic in nics:
                try:
                    ...
                    if (
                        hasattr(nic_details, "ip_configurations")
                        and nic_details.ip_configurations
                    ):
                        for ip_config in nic_details.ip_configurations:
                            if (
                                hasattr(ip_config, "subnet")
                                and ip_config.subnet
                                and ip_config.subnet.id
                            ):
                                ...
                                if (
                                    ...
                                    if vnet_index + 1 < len(
                                        ...
                                        nic.add_dependency(
                                            subnet_full_name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - NIC subnet placement",
                                        )
                                        logger.debug(
                                            f"Added subnet dependency: {nic.name} -> {subnet_full_name}",
                                        )
    ...
    def _add_internet_and_discover_nsg_relationships(
        ...
        try:
            ...
            nsgs = [
                r
                for r in resources
                if r.resource_type == "Microsoft.Network/networkSecurityGroups"
            ]
            subnets = [
                r
                for r in resources
            ...
            if public_ips:
                ...
                for pip in public_ips:
                    internet_resource.add_dependency(
                        pip.name,
                        DependencyType.EXPLICIT,
                        "Virtual - internet gateway to public IP",
                    )
                                ...
                                if (
                                    ...
                                    if vnet_index + 1 < len(
                                        ...
                                        nsg.add_dependency(
                                            subnet_full_name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - NSG subnet association",
                                        )
                                        logger.debug(
                                            f"Added subnet dependency: {nsg.name} -> {subnet_full_name}",
                                        )
        ...
        try:
            ...
            for vm in vms:
                try:
                    ...
                    if (
                        hasattr(vm_details, "diagnostics_profile")
                        and vm_details.diagnostics_profile
                        and hasattr(vm_details.diagnostics_profile, "boot_diagnostics")
                        and vm_details.diagnostics_profile.boot_diagnostics
                        and hasattr(
                            vm_details.diagnostics_profile.boot_diagnostics,
                            "storage_uri",
                        )
                                ...
                                for sa in storage_accounts:
                                    if sa.name == storage_name:
                                        vm.add_dependency(
                                            sa.name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - VM boot diagnostics storage",
                                        )
                                        logger.debug(
                                            f"Added storage dependency: {vm.name} -> {sa.name} (boot diagnostics)",
                                        )
                                        break

                    # Check for unmanaged disks (if any VMs still use them)
                    if (
                        hasattr(vm_details, "storage_profile")
                        and vm_details.storage_profile
                        and hasattr(vm_details.storage_profile, "os_disk")
                        and vm_details.storage_profile.os_disk
                        and hasattr(vm_details.storage_profile.os_disk, "vhd")
                                ...
                                for sa in storage_accounts:
                                    if sa.name == storage_name:
                                        vm.add_dependency(
                                            sa.name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - VM unmanaged disk storage",
                                        )
                                        logger.debug(
                                            f"Added storage dependency: {vm.name} -> {sa.name} (unmanaged disk)",
                                        )
                                        break
            ...
            for sa in storage_accounts:
                ...
                for vm in vms:
                    ...
                    if len(vm_base) >= 3 and len(sa_base) >= 3:
                        ...
                        if (
                            vm_base.startswith(sa_base[:6])
                            or sa_base.startswith(vm_base[:6])
                            or vm_base in sa_base
                            or sa_base in vm_base
                        ):
                            ...
                            if sa.name not in vm.get_dependency_names():
                                ...
                                logger.debug(
                                    f"Added storage dependency: {vm.name} -> {sa.name} (naming pattern match)",
                                )
                        ...
                        # Method 2: Check for common prefixes (like "win" in both "WIN-ansible" and "winansible4697")
                            ...
                            if vm_prefix == sa_prefix:
                                if sa.name not in vm.get_dependency_names():
                                    ...
                                    logger.debug(
                                        f"Added storage dependency: {vm.name} -> {sa.name} (prefix pattern match)",
                                    )
            ...
            if storage_accounts and vms:
                ...
                for sa in storage_accounts:
                    ...
                    sa_name_lower = sa.name.lower()
                    if any(
                        keyword in sa_name_lower
                        for keyword in ["cluster", "registry", "image"]
                    ):
                    ...
                    # Extract potential cluster identifier from resource group or VM names
                        ...
                        for vm in vms:
                            if (
                                "master" in vm.name.lower()
                                or "worker" in vm.name.lower()
                            ):
                                # Extract cluster name from VM name (e.g., "byoid-fp64f" from "byoid-fp64f-master-0")
                                parts = vm.name.lower().split("-")
                                if (
                                    len(parts) >= 3
                                ):  # e.g., ["byoid", "fp64f", "master", "0"]
                                    potential_cluster_name = "-".join(
                                        parts[:-2],
                                    )  # "byoid-fp64f"
                                    # Remove hyphens for storage account name comparison
                                    cluster_name_no_hyphen = (
                                        potential_cluster_name.replace("-", "")
                                    )
                        ...
                        for prefix in vm_prefixes:
                            if (
                                prefix in sa_name_lower and len(prefix) > 3
                            ):  # Avoid short/common prefixes
                                cluster_storage_accounts.append(sa)
                                logger.debug(
                                    f"Storage account '{sa.name}' identified as cluster storage based on naming pattern '{prefix}'",
                                )
                                break
                    ...
                    if master_vms:
                        ...
                        for master_vm in master_vms:
                            for sa in cluster_storage_accounts:
                                master_vm.add_dependency(
                                    sa.name,
                                    DependencyType.DERIVED,
                                    "cluster storage pattern",
                                )
                                logger.debug(
                                    f"Added cluster storage dependency: {master_vm.name} -> {sa.name} (cluster storage)",
                                )
                    else:
                        ...
                        controller_vms = [
                            ...
                            for vm in vms
                            if any(
                                keyword in vm.name.lower()
                                for keyword in ["control", "manage"]
                            )
                        ]
                        if controller_vms:
                            ...
                            for controller_vm in controller_vms:
                                for sa in cluster_storage_accounts:
                                    controller_vm.add_dependency(
                                        sa.name,
                                        DependencyType.DERIVED,
                                        "cluster storage pattern",
                                    )
                                    logger.debug(
                                        f"Added cluster storage dependency: {controller_vm.name} -> {sa.name} (cluster storage)",
                                    )
        ...
        try:
            ...
            for subnet in subnets:
                if "vnet_name" in subnet.properties:
                    ...
                    created_vnets[vnet_name].add_dependency(
                        subnet.name,
                        DependencyType.EXPLICIT,
                        "Azure API - VNet subnet containment",
                    )
                    logger.debug(
                        f"Added subnet dependency: {vnet_name} -> {subnet.name}",
                    )
    ...
    def _discover_openshift_cluster_relationships(
        ...
        openshift_clusters = [
            r
            for r in resources
            if r.resource_type == "Microsoft.RedHatOpenShift/OpenShiftClusters"
        ]
        ...
        try:
            for cluster in openshift_clusters:
                try:
                    logger.info(
                        f"Discovering dependencies for OpenShift cluster: {cluster.name}",
                    )
                    ...
                    for vnet in vnets:
                        ...
                        if (
                            cluster_name_base in vnet_name_lower
                            or (
                                vnet_name_lower.startswith("vnet-")
                                and cluster_name_base in vnet_name_lower
                            )
                            or "openshift" in vnet_name_lower
                        ):
                            cluster.add_dependency(
                                vnet.name,
                                DependencyType.DERIVED,
                                "cluster name pattern match",
                            )
                            logger.info(
                                f"Added VNet dependency: {cluster.name} -> {vnet.name} (derived)",
                            )
                    ...
                    for subnet in subnets:
                        ...
                        if (
                            cluster_name_base in subnet_name_lower
                            or "master" in subnet_name_lower
                            or "worker" in subnet_name_lower
                            or "openshift" in subnet_name_lower
                            or "aro" in subnet_name_lower
                        ):
                            cluster_subnets.append(subnet)
                            cluster.add_dependency(
                                subnet.name,
                                DependencyType.DERIVED,
                                "cluster subnet pattern match",
                            )
                            logger.info(
                                f"Added subnet dependency: {cluster.name} -> {subnet.name} (derived)",
                            )
                    ...
                    for sa in storage_accounts:
                        sa_name_lower = sa.name.lower()
                        if (
                            "registry" in sa_name_lower
                            or "cluster" in sa_name_lower
                            or cluster_name_base.replace("-", "") in sa_name_lower
                            or "openshift" in sa_name_lower
                            or "aro" in sa_name_lower
                        ):
                            cluster.add_dependency(
                                sa.name,
                                DependencyType.DERIVED,
                                "cluster storage pattern match",
                            )
                            logger.info(
                                f"Added storage dependency: {cluster.name} -> {sa.name} (derived)",
                            )
                    ...
                    for vm in vms:
                        ...
                        if (
                            cluster_name_base in vm_name_lower
                            or ("master" in vm_name_lower and "aro" in vm_name_lower)
                            or ("worker" in vm_name_lower and "aro" in vm_name_lower)
                            or "openshift" in vm_name_lower
                        ):
                            cluster_vms.append(vm)
                            cluster.add_dependency(
                                vm.name,
                                DependencyType.DERIVED,
                                "cluster VM pattern match",
                            )
                            logger.info(
                                f"Added VM dependency: {cluster.name} -> {vm.name} (derived)",
                            )
                    ...
                    for nic in nics:
                        ...
                        if (
                            cluster_name_base in nic_name_lower
                            or "master" in nic_name_lower
                            or "worker" in nic_name_lower
                            or "openshift" in nic_name_lower
                            or "aro" in nic_name_lower
                        ):
                            cluster.add_dependency(
                                nic.name,
                                DependencyType.DERIVED,
                                "cluster NIC pattern match",
                            )
                            logger.info(
                                f"Added NIC dependency: {cluster.name} -> {nic.name} (derived)",
                            )
                    ...
                    for lb in load_balancers:
                        ...
                        if (
                            cluster_name_base in lb_name_lower
                            or "openshift" in lb_name_lower
                            or "aro" in lb_name_lower
                        ):
                            cluster.add_dependency(
                                lb.name,
                                DependencyType.DERIVED,
                                "cluster LB pattern match",
                            )
                            logger.info(
                                f"Added load balancer dependency: {cluster.name} -> {lb.name} (derived)",
                            )

                    # Set cluster properties for better visualization
                    if cluster.dependencies:
                        cluster.properties["has_dependencies"] = True
                        cluster.properties["dependency_count"] = len(
                            cluster.dependencies,
                        )
                        logger.info(
                            f"OpenShift cluster {cluster.name} now has {len(cluster.dependencies)} dependencies",
                        )
                    else:
                        logger.warning(
                            f"No dependencies found for OpenShift cluster {cluster.name}",
                        )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze OpenShift cluster '{cluster.name}': {e}",
                    )
                    continue
    ...
    def _discover_solutions_application_relationships(
        ...
        solutions_apps = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Solutions/applications"
        ]
        ...
        try:
            for app in solutions_apps:
                try:
                    logger.info(
                        f"Discovering dependencies for Solutions application: {app.name}",
                    )
                    ...
                    if (
                        app_details
                        and hasattr(app_details, "properties")
                        and app_details.properties
                    ):
                        ...
                        if (
                            hasattr(props, "managed_resource_group_id")
                            and props.managed_resource_group_id
                        ):
                            managed_rg_id = props.managed_resource_group_id
                        elif (
                            hasattr(props, "managedResourceGroupId")
                            and props.managedResourceGroupId
                        ):
                        ...
                        # Method 2: Dictionary-like access if props is dict-like
                            ...
                            try:
                                managed_rg_id = props.get(
                                    "managedResourceGroupId",
                                ) or props.get("managed_resource_group_id")
                            except:
                                pass

                        # Method 3: additional_properties
                        elif (
                            hasattr(props, "additional_properties")
                            and "managedResourceGroupId" in props.additional_properties
                        ):
                            managed_rg_id = props.additional_properties[
                                "managedResourceGroupId"
                            ]

                        logger.debug(
                            f"Managed resource group ID found: {managed_rg_id}",
                        )

                        if managed_rg_id:
                            # Extract resource group name from the ID
                            managed_rg_name = self._extract_resource_group_name_from_id(
                                managed_rg_id,
                            )
                            logger.debug(
                                f"Found managed resource group ID: {managed_rg_id} -> {managed_rg_name}",
                            )

                            if managed_rg_name:
                                logger.info(
                                    f"Solutions app {app.name} manages resource group: {managed_rg_name}",
                                )
                                ...
                                try:
                                    ...
                                    managed_subscription_id = managed_rg_id.split("/")[
                                        2
                                    ]
                                    logger.debug(
                                        f"Managed resource group is in subscription: {managed_subscription_id}",
                                    )
                                    ...
                                    for managed_resource in managed_resources:
                                        ...
                                        app.add_dependency(
                                            ...
                                            DependencyType.EXPLICIT,
                                            "Azure API - Solutions app managed resource",
                                        )
                                        managed_count += 1
                                        logger.debug(
                                            f"Added managed resource dependency: {app.name} -> {managed_resource_name}",
                                        )

                                    if managed_count > 0:
                                        app.properties["managed_resource_group"] = (
                                            managed_rg_name
                                        )
                                        app.properties["managed_resource_count"] = (
                                            managed_count
                                        )
                                        logger.info(
                                            f"Solutions app {app.name} now manages {managed_count} resources in {managed_rg_name}",
                                        )

                                except Exception as e:
                                    logger.warning(
                                        f"Could not list resources in managed resource group '{managed_rg_name}': {e}",
                                    )
                        ...
                        if hasattr(props, "outputs") and props.outputs:
                            ...
                            logger.debug(
                                f"Solutions app {app.name} has outputs: {list(props.outputs.keys())}",
                            )
                        ...
                        if hasattr(props, "parameters") and props.parameters:
                            ...
                            logger.debug(
                                f"Solutions app {app.name} has parameters: {list(param_info.keys())}",
                            )
                    ...
                    if app.dependencies:
                        ...
                        logger.info(
                            f"Solutions application {app.name} now has {len(app.dependencies)} dependencies",
                        )
                    else:
                        logger.warning(
                            f"No managed resources found for Solutions application {app.name}",
                        )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze Solutions application '{app.name}': {e}",
                    )
                    continue

        except Exception as e:
            logger.warning(
                f"Failed to discover Solutions application relationships: {e}",
            )
    ...
    def _discover_application_gateway_relationships(
        ...
        app_gateways = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Network/applicationGateways"
        ]
        ...
        try:
            for app_gw in app_gateways:
                try:
                    logger.info(
                        f"Discovering dependencies for Application Gateway: {app_gw.name}",
                    )
                    ...
                    if (
                        hasattr(app_gw_details, "firewall_policy")
                    ...
                    if (
                        hasattr(app_gw_details, "frontend_ip_configurations")
                        and app_gw_details.frontend_ip_configurations
                    ):
                        for frontend_ip in app_gw_details.frontend_ip_configurations:
                            if (
                                hasattr(frontend_ip, "public_ip_address")
                    ...
                    if (
                        hasattr(app_gw_details, "gateway_ip_configurations")
                        and app_gw_details.gateway_ip_configurations
                    ):
                        for (
                            gateway_ip_config
                        ) in app_gw_details.gateway_ip_configurations:
                            if (
                                hasattr(gateway_ip_config, "subnet")
                    ...
                    if app_gw.dependencies:
                        ...
                        logger.info(
                            f"Application Gateway {app_gw.name} now has {len(app_gw.dependencies)} dependencies",
                        )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze Application Gateway '{app_gw.name}': {e}",
                    )
                    continue
    ...
    def _discover_vm_extension_relationships(
        ...
        vm_extensions = [
            r
            for r in resources
            if r.resource_type == "Microsoft.Compute/virtualMachines/extensions"
        ]
        ...
        try:
            for vm_ext in vm_extensions:
                try:
                    logger.info(
                        f"Discovering dependencies for VM extension: {vm_ext.name}",
                    )
                    ...
                    if "/" in vm_ext.name:
                        ...
                        for vm in vms:
                            if vm.name == vm_name:
                                vm_ext.add_dependency(
                                    vm.name,
                                    DependencyType.EXPLICIT,
                                    "Azure API - VM extension parent VM",
                                )
                                logger.debug(
                                    f"Added VM dependency: {vm_ext.name} -> {vm.name}",
                                )
                                break
                    ...
                    if vm_ext.dependencies:
                        ...
                        logger.info(
                            f"VM extension {vm_ext.name} now has {len(vm_ext.dependencies)} dependencies",
                        )

                except Exception as e:
                    logger.warning(
                        f"Could not analyze VM extension '{vm_ext.name}': {e}",
                    )
                    continue
    ...
    def get_network_topology(
        ...
        try:
            ...
            if not network_watcher:
                logger.debug(
                    f"No Network Watcher found for location '{location}', skipping network topology",
                )
    ...
    def _parse_network_topology(self, topology: Any) -> NetworkTopology:
        ...
        for resource in topology.resources:
            ...
            try:
                ...
                logger.debug(
                    f"Could not determine resource type for resource: {resource}",
                )
                continue
        ...
        try:
            ...
            for resource in resources:
                ...
                if "external_pls_connections" in resource.properties:
                    logger.info(
                        f"Processing external connections for {resource.name}: {len(resource.properties['external_pls_connections'])} connections",
                    )
                    for conn in resource.properties["external_pls_connections"]:
                        external_resource_ids.add(conn["id"])
                        logger.info(
                            f"Found external PLS dependency: {resource.name} -> {conn['name']} (RG: {conn['resource_group']})",
                        )

                # Check for Solutions application managed resources
                if (
                    resource.resource_type == "Microsoft.Solutions/applications"
                    and "managed_resource_group" in resource.properties
                ):
                    managed_rg = resource.properties["managed_resource_group"]
                    logger.info(
                        f"Processing managed resources for Solutions app {resource.name}: managed RG {managed_rg}",
                    )
                    ...
                    try:
                        ...
                        for managed_res in managed_resources:
                            ...
                            logger.info(
                                f"Found managed resource dependency: {resource.name} -> {managed_res.name} (RG: {managed_rg})",
                            )

                    except Exception as e:
                        logger.warning(
                            f"Could not list managed resources for Solutions app {resource.name}: {e}",
                        )

            logger.info(
                f"Total external resource IDs to process: {len(external_resource_ids)}",
            )
            ...
            for resource_id in external_resource_ids:
                ...
                if external_resource:
                    ...
                    logger.info(
                        f"Added cross-RG dependency: {external_resource.name} from {external_resource.resource_group}",
                    )
                            ...
                            for conn in resource.properties["external_pls_connections"]:
                                if conn["id"] == resource_id:
                                    resource.add_dependency(
                                        ...
                                        DependencyType.EXPLICIT,
                                        "Azure API - cross-resource group connection",
                                    )
                                    logger.debug(
                                        f"Added dependency: {resource.name} -> {external_resource.name}",
                                    )
                else:
                    ...
                    logger.info(
                        f"External resource fetch failed for {resource_id}, creating placeholder (cross-tenant: {is_cross_tenant})",
                    )

                    # Create a placeholder resource for visualization
                    placeholder_resource = self._create_placeholder_external_resource(
                        resource_id,
                        is_cross_tenant,
                        "Could not access external resource",
                    )
                    if placeholder_resource:
                        resources.append(placeholder_resource)
                        logger.info(
                            f"Added placeholder for external dependency: {placeholder_resource.name} from {placeholder_resource.resource_group} (cross-tenant: {is_cross_tenant})",
                        )

                        # Create dependencies to placeholder
                        for resource in resources:
                            if "external_pls_connections" in resource.properties:
                                for conn in resource.properties[
                                    "external_pls_connections"
                                ]:
                                    if conn["id"] == resource_id:
                                        resource.add_dependency(
                                            placeholder_resource.name,
                                            DependencyType.EXPLICIT,
                                            "Azure API - cross-resource group connection (placeholder)",
                                        )
                                        logger.info(
                                            f"Added dependency to placeholder: {resource.name} -> {placeholder_resource.name}",
                                        )
                    else:
                        logger.warning(
                            f"Failed to create placeholder for {resource_id}",
                        )
    ...
    def _fetch_external_resource(self, resource_id: str) -> Optional[AzureResource]:
        ...
        try:
            ...
            logger.debug(
                f"Fetching external resource: {resource_name} of type {full_type} from RG {resource_group}",
            )
            ...
            if "NoRegisteredProviderFound" in error_msg:
                logger.debug(
                    f"External resource {resource_name} ({full_type}) not accessible due to provider registration - creating placeholder",
                )
            elif "ResourceNotFound" in error_msg:
                logger.debug(
                    f"External resource {resource_name} not found - may have been deleted",
                )
            elif "Forbidden" in error_msg or "Unauthorized" in error_msg:
                logger.debug(
                    f"External resource {resource_name} not accessible due to permissions",
                )
            else:
                logger.warning(
                    f"Could not fetch external resource {resource_name} ({full_type}): {e}",
                )
            return None
        except Exception as e:
    ...
    def _create_placeholder_external_resource(
        self,
        resource_id: str,
        ...
        try:
            ...
            if len(id_parts) < 9:
                logger.warning(
                    f"Invalid resource ID format for placeholder: {resource_id}",
                )
                return None
            ...
            logger.warning(
                f"Error creating placeholder for resource ID {resource_id}: {e}",
            )
            return None
    ...
    def _extract_openshift_dns_configuration(self, cluster: AzureResource) -> None:
        ...
        try:
            ...
            if (
                "apiserverProfile" in cluster_properties
                and "url" in cluster_properties["apiserverProfile"]
            ):
                ...
                if "://" in api_url:
                    ...
                    logger.info(
                        f"Extracted API DNS domain for {cluster.name}: {domain_part}",
                    )

            # From console profile
            if (
                "consoleProfile" in cluster_properties
                and "url" in cluster_properties["consoleProfile"]
            ):
                ...
                if "://" in console_url:
                    ...
                    dns_domains.append(domain_part)
                    logger.info(
                        f"Extracted console DNS domain for {cluster.name}: {domain_part}",
                    )

            # From cluster profile domain
            if (
                "clusterProfile" in cluster_properties
                and "domain" in cluster_properties["clusterProfile"]
            ):
                cluster_domain = cluster_properties["clusterProfile"]["domain"]
                dns_domains.append(cluster_domain)
                logger.info(
                    f"Extracted cluster domain for {cluster.name}: {cluster_domain}",
                )
            ...
            if "ingressProfiles" in cluster_properties:
                for ingress_profile in cluster_properties["ingressProfiles"]:
                    if "domain" in ingress_profile:
                        ...
                        logger.info(
                            f"Found custom ingress domain for {cluster.name}: {custom_domain}",
                        )

                    # Check for wildcard domains in ingress configuration
                    if "wildcardPolicy" in ingress_profile:
                        logger.info(
                            f"Ingress wildcard policy for {cluster.name}: {ingress_profile['wildcardPolicy']}",
                        )

            # Store extracted DNS domains in cluster properties
            if dns_domains:
                cluster.properties["openshift_dns_domains"] = list(
                    set(dns_domains),
                )  # Remove duplicates
                logger.info(
                    f"Stored {len(set(dns_domains))} DNS domains for OpenShift cluster {cluster.name}: {list(set(dns_domains))}",
                )
                ...
                if (
                    "apiserverProfile" in cluster_properties
                    and "ip" in cluster_properties["apiserverProfile"]
                ):
                ...
                logger.warning(
                    f"No DNS domains found for OpenShift cluster {cluster.name}",
                )
